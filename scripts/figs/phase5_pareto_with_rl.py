...
"""Overlay the RL agents on the Phase-3 Pareto projections.

Reads ``results/sweep_phase3/cells.csv`` + ``reflex_aggregate.csv`` as the
backdrop, then reads the Phase-5 held-out ``metrics.csv`` to extract per-
agent means for Reflex-held-out, Tuned-held-out, and each RL run. Re-plots
the three Pareto projections (completion_vs_cost, completion_vs_failure,
cost_vs_failure) with the held-out RL points marked.

Coloured star markers for the three RL runs; black diamond for the
held-out Reflex mean (which should coincide closely with the 20-seed
Reflex in the sweep); blue cross for the tuned-UB held-out mean.
"""
...
def _held_out_agent_means(run_dir: Path) -> Dict[str, Dict[str, float]]:
...
def main() -> None:
    ...
    heldout = _held_out_agent_means(args.heldout_dir)

    fig, axes = plt.subplots(1, 3, figsize=(18, 5.6), constrained_layout=True)

    rl_palette = ["#c92a2a", "#e67700", "#845ef7"]
    rl_agents = sorted([a for a in heldout if a.startswith(args.rl_label_prefix)])
    tuned_agents = sorted([a for a in heldout if a.startswith(args.tuned_label_prefix)])
    reflex_held = next((a for a in heldout if a == "Reflex Agent"), None)
    ...
    for ax, (projection_name, pareto_axes, metric_pair, title) in zip(axes, PROJECTIONS):
        ...
        ax.scatter(
            ...
            color="black",
            s=110,
            label="Reflex (sweep pool, weight-indep.)",
            zorder=6,
        )

        if reflex_held is not None:
            ax.scatter(
                heldout[reflex_held][x_key_held],
                heldout[reflex_held][y_key_held],
                marker="D",
                color="#495057",
                s=100,
                edgecolor="black",
        ...
        for agent in tuned_agents:
            ax.scatter(
                heldout[agent][x_key_held],
                heldout[agent][y_key_held],
                marker="X",
                color="#1c7ed6",
                s=150,
                edgecolor="black",
                linewidths=0.5,
                label="Tuned Utility-Based (held-out)",
                zorder=7,
            )

        for agent, colour in zip(rl_agents, rl_palette):
            ax.scatter(
                heldout[agent][x_key_held],
                heldout[agent][y_key_held],
                marker="*",
    ...
    fig.suptitle(
        "Phase-5 RL held-out points overlaid on the Phase-3 Pareto projections",
        fontsize=13,
        fontweight="bold",
    )
